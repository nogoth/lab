...
async def talkToServer():
    ...
    async with client:
        ...
        prompt_template = ChatPromptTemplate.from_messages( [ SystemMessage(content=SYSTEM_PROMPT2), # <-- Insert the new prompt here
        ...
        # start the story
        print("Start the story") #seems the content of the message really really makes things weird
        final_state = await app.ainvoke({"messages": [HumanMessage(content="Generate the first two lines of a story")]})
        print(final_state)
